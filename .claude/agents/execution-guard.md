...
| --- | --- | --- | --- | --- |
| 433736 | SIMULATE | CASH | HK | ○（確認5をクリアできるが、US銘柄は扱えない） |
| 433735 | SIMULATE | MARGIN | US | ×（acc_type=MARGINのため確認5で必ず拒否） |

**米国市場移行後の未解決の制約（最重要）**

対象市場は米国（NASDAQ/NYSE）になりましたが、**US銘柄を扱える口座(433735)は
MARGIN（信用口座）であり、確認5「信用取引でないこと」を通過できません。**
一方、確認5を通過できる433736はHK専用でUS銘柄を扱えません。

したがって**現時点で米国株をこのゲート経由で実発注することはできません。**
US銘柄の発注を求められた場合は、確認を進める前にこの制約を利用者に報告してください。
確認1〜4（停止指示・重複発注・ポジション上限・日次損失上限）はAPI接続なしで
動作するため、ロジックの検証には引き続き使えます。

CASHのUS口座が用意されたら、この表と `scripts/execution_guard.py` の
`DEFAULT_ACC_ID` を併せて更新してください。**確認5を緩めて回避してはいけません。**
...
## 報告形式
    ...
    ※ 対象銘柄が米国株(US)の場合、確認5を通過できる口座が存在しないため
      この時点で「US銘柄を扱える口座がMARGINのみのため発注できません」と
      報告してください（上記「口座と市場の制約」参照）。
